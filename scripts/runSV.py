...
def getArgs() :    
    ...
    parser.add_argument('-s', help='SV jobs configuration: 0 = single job (required for correct haplotype calls), 1 = single job per contig (not recommended), 2 = grouped (default 0; optional)', dest='groupsv', type=int, default=0)
...
def runSV(cwd, rabin, refcmap, contigdir, contigbase, runaligns, xmappath, optargs, nthreads, maxthreads, bedfile, errfile, outdir, errbinfile, clustargs, groupsv):
    ...
    if runaligns :
        ...
        cmaps = util.getListOfFilesFromDir(varsP.outputContigFolder, suffix=".cmap")
        if len(cmaps) > 1 :
