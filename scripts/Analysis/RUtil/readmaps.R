...
load_err <- function(filename) {
    A <- read.table(filename, comment.char="#", header=FALSE, stringsAsFactors=FALSE, sep="")
    #for .err, there is no "#h", instead, first row is header
    h <- A[1,]
    A <- A[-1,]
    names(A) <- h
    return(A)
...
load_map <- function(filename, sep="") {
  ...
  A <- read.table(filename, comment.char="#", header=FALSE, stringsAsFactors=FALSE, sep=sep)
  #print(A[1:10,])
  ...
  #this block converts strings to factors; don't know why it's needed, just remove nrows arg to read.table above
  #if(dim(A)[1]>=5000) {
