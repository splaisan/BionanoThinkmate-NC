...
util.setVersion("$Id: SVModule.py 5134 2016-07-07 00:51:33Z wandrews $")
...
class SVdetect(mthread.jobWrapper):
    ...
    def __init__(self, varsP, noisep={}, fixedoutputdir="", skipderes=False): #targetStage, 
        ...
        self.largemem = "largeNodeMem" #if group == 0, always use this section in optargs
        ...
        ca = self.varsP.getClusterArgs(self.moduleName if self.group_jobs > 0 else "assembly")
    ...
    def generateJobList(self) :
        ...
        maxm = '-maxmem'
        if group == 0 and maxm in endargs and self.varsP.argData.has_key(self.largemem):
            largs = self.varsP.argsListed(self.largemem) #should be just ['-maxmem', '128']
            if len(largs) == 2 and util.getIntFromString(largs[1]) != None :
                self.varsP.updatePipeReport("SVDetect: chaning -maxmem from "+endargs[endargs.index(maxm)+1]+" to "+largs[1]+" due to grouping\n")
